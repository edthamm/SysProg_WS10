...
@detail This bot connects to a gameserver and plays a simple game of dice against 2 opponents remembering his own and their scores and reporting them on end of game. Terminates with 0 on success and 1 on failure. For detail see STDERR.
@date 04.04.2011
...
static void play(void){
	...
	if((srvin = fdopen(sid,"r")) == NULL){
		(void)fprintf(stderr, "ERROR opening socket as filestream.");
		(void)(cleanup);
		/*you still need to close this*/
	}

	while(1==1){
		/*(void)read(sid,buffer,127);*/

		(void)fgets(buffer,127,srvin);
		printf("RCV: %s\n",buffer);
		...
		if(strncmp(buffer,"TURN",4) == 0){
			...
			while(i>0){
				...
				if(strncmp(buffer,"THRW",4) == 0){
					strncpy(id,&buffer[5],1);; id[1]='\0';		
					strncpy(d1,&buffer[7],1); d1[1]='\0';
					strncpy(d2,&buffer[9],1);; d2[1]='\0';
		...
		if(strncmp(buffer,"THRW",4) == 0){
			strncpy(id,&buffer[5],1);; id[1]='\0';/*\0 termination needed for all strings or strange things will happen*/
			strncpy(d1,&buffer[7],1);; d1[1]='\0';
			strncpy(d2,&buffer[9],1);; d2[1]='\0';
			l = strtol(d1,NULL,10);
			/* l = d1 - "0"; */
